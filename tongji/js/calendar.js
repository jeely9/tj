...
function getWeekNumber(y, m, d) {
	...
	var week = null;
	if(yearFirstDay == 1) {
		week = Math.floor(days / yearFirstDay);
	} else {
...
(function($, window, document, undefined) {
	...
	Calendar.prototype = {
		showHoverInfo: function(obj) { // hover 时显示当天信息
			...
			this_week = getWeekNumber(_dateStr.substr(0, 4), _dateStr.substr(4, 2), _dateStr.substring(6));
			/*ajax请求后台数据*/
			function weekAjax() {
				$.ajax({
					type: "post",
					url: "../../../../TongjiUniversity/hot/historyWeekly.do",
					async: true,
			...
			weekAjax();
			function historyWeekly(jsonData) {
				$(".date").empty();
				$(".week").empty();
				for(var i=0;i<jsonData.length;i++){
					if(this_week == jsonData[i].numWeek){
						changeStr = _dateStr.substr(0, 4)+"年第"+this_week+"周";
						_weekStr = jsonData[i].keyword;
						$(".date").html(changeStr);
						$(".week").html(_weekStr);
					}
				}
		...
		renderDOM: function() { // 渲染DOM
			...
			var _titleStr = '<a href="#" class="date_title"></a>' +
		...
		inital: function() { // 初始化
			...
			this.$calendarTitle_text = this.$calendar_title.find('.date_title');
			...
			this.$calendarToday_date = [];
			this.$calendarToday_week = [];
